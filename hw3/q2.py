...
def setup_fuzzy_controller():
    ...
    error = ctrl.Antecedent(np.arange(-10, 31, 0.1), 'error')  # Speed error (target speed - current speed)
    derivative = ctrl.Antecedent(np.arange(-10, 11, 0.1), 'derivative')  # Rate of change of error
    control_signal = ctrl.Consequent(np.arange(-0.1, 1, 0.01), 'control_signal')  # Throttle control (bounded between -0.1 and 1)

    
    # Define membership functions for error
    error['NB'] = fuzz.trimf(error.universe, [-10, -10, 0])
    error['NS'] = fuzz.trimf(error.universe, [-10, 0, 2.5])
    error['Z'] = fuzz.trimf(error.universe, [0, 2.5, 10])
    error['PS'] = fuzz.trimf(error.universe, [0, 15, 30])
    error['PB'] = fuzz.trimf(error.universe, [15, 30, 30])
    
    # Define membership functions for derivative (rate of change of error)
    derivative['NB'] = fuzz.trimf(derivative.universe, [-10, -10, -5])
    derivative['NS'] = fuzz.trimf(derivative.universe, [-5, 0, 5])
    derivative['Z'] = fuzz.trimf(derivative.universe, [-2.5, 0, 2.5])
    derivative['PS'] = fuzz.trimf(derivative.universe, [0, 5, 10])
    derivative['PB'] = fuzz.trimf(derivative.universe, [5, 10, 10])
    
    # Define membership functions for control signal (throttle control)
    control_signal['NB'] = fuzz.trimf(control_signal.universe, [-0.1, -0.1, 0])
    control_signal['NS'] = fuzz.trimf(control_signal.universe, [-0.1, -0.05, 0])
    control_signal['Z'] = fuzz.trimf(control_signal.universe, [-0.05, 0, 0.5])
    control_signal['PS'] = fuzz.trimf(control_signal.universe, [0, 0.5, 1])
    control_signal['PB'] = fuzz.trimf(control_signal.universe, [0, 1, 1])
...
def fuzzy_control(fuzzy_controller, error, derivative):
    print(f"Running fuzzy control with error: {error}, derivative: {derivative}")
    # Assign inputs
    ...
    print(f"Fuzzy Controller Inputs: {fuzzy_controller.input}")
    print(f"Fuzzy Controller Outputs: {fuzzy_controller.output}")
    # Check output after computation
    if 'control_signal' not in fuzzy_controller.output:
        print(f"Error: 'control_signal' not found in fuzzy controller output.")
        return None
    else:
        print(f"Control signal: {fuzzy_controller.output['control_signal']}")
...
target_speed = 30  # Target speed in m/s (e.g., 20 m/s = 72 km/h)
